...
In Section~\ref{sec:backaction} I show the general mechanism by which the divergence of trajectories decoheres internal states of an atom, and specifically how this appears as an apparent decrease in the amplitudes of each other state when one imposes the adiabatic trajectory corresponding to one specific internal state. I then show in Section~\ref{sec:zeno_effect} why naively projecting the wavefunction onto a single trajectory at all times yields nonsensical results, and then in Subsections~\ref{sec:markovian_decoherence} and~\ref{sec:markovian_decoherence} I present two remedies for how to approximately include decoherence despite this. 
...
The (negative) logarithmic derivative of the decoherence factor for our fixed-width Gaussian wavepackets~\eqref{eq:decoherence_factor} gives the decoherence rate
\begin{align}
...
Now we have presented all the pieces from which the two versions of the hidden variables semiclassical method are constructed. One more approximation is yet to be mentioned, and that is that the projected Hamiltonian $\hat H_\eta(t)$ first introduced in Section~\ref{sec:backaction} is approximated by the value of the full Hamiltonian at the centre-of-mass position of the main trajectory's Gaussian wavepacket, without computing an integral:
\begin{align}
...
\end{align}
If projecting onto a motional state other than $\ket{\phi_\eta}$, we use the position of that motional state instead. Projecting the Hamiltonian onto an arbitrary position state $\ket{\vec r}$ gives an operator valued function of space (and time) $\hat H(\vec r, t)$, which operates only on the internal degrees of freedom of the atom.

With this final approximation, there are no longer any Guassian wavepackets to compute integrals over or anything else---all details of the wavepackets have been encapsulated and parametrised by the approximations and analytics in the preceding sections, and we can now focus on the classical dynamics of them atoms' centre-of-mass motion and the quantum evolution of their internal states according to $\hat H(\vec r, t)$ evaluated at the classical positions (representing the centre-of-mass position of the wavepackets) of the atoms.
...
\subsection{Mean auxiliary trajectories hidden-variable semiclassical method}
