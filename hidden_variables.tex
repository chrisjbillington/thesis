...
We begin with the time dependent Schr\"odinger equation for the internal state $\ket{\chi(t)}$ of an atom, assuming it is well localised at position $\vec r$:
\begin{align}
...
\end{align}
where $\hat V(\vec r, t)$ is a diagonal operator with diagonals equalling the adiabatic potentials that each eigenstate of $\hat H$ is subject to in the adiabatic approximation.
We can therefore define the state vector in the eigenbasis of $H$ as:\footnote{This is very similar to an interaction picture state vector [SEE SECREF], but as I have previously used the definition of an interaction picture as the transformation that diagonalises a \emph{time-independent} Hamiltonian, this potentially time-dependent Hamiltonian does not satisfy the definition.}

\begin{align}
\ket{\chi_H(t)} &= \hat U_H(\vec r, t)\ket{\chi(t)}\\
\Rightarrow \ket{\chi(t)} &= \hat U^\dagger_H(\vec r, t)\ket{\chi_H(t)},\label{eq:chi_Hbasis}
\end{align}
...
\begin{align}
\dv{t}U^\dagger_H(\vec r, t)
 = \pdv{t} U^\dagger_H(\vec r, t) + \vec v\cdot \nabla U^\dagger_H(\vec r, t),
\end{align}
...
\ii\hbar\dv{t}\ket{\chi_H(t)} = \left[
 ...
 \right]\ket{\chi_H(t)}.
\end{align}
The final term in brackets is identical to the non-adiabatic coupling term in the equation of motion as usually written in the surface-hopping literature [CITE SOME THINGS] being a matrix with elements:
\begin{align}
\left(- \ii\hbar\vec v\cdot \hat U(\vec r, t)\nabla U^\dagger_H(\vec r, t)\right)_{ij}
= - \ii\hbar\vec v\cdot\matrixel{\chi_i(\vec r, t)}{\nabla}{\chi_j (\vec r, t)},
\end{align}
where $\chi_i(\vec r, t)$ is the $i^\up{th}$ eigenvector of $\hat H(r, t)$ and $\matrixel{\chi_i(\vec r, t)}{\nabla}{\chi_j (\vec r, t)}$ is the \emph{non-adiabatic coupling vector} between the $i^\up{th}$ and $j^\up{th}$ states referred to in the literature. The second to last term in brackets in \eqref{eq:Tully_adiabatic_schro}
...
\end{align}
where via~\eqref{eq:Tully_adiabatic_schro} we have:
\begin{align}
...
\sum_j\re\left(c_i^*(t)
 ...
 c_j(t)\right).
\end{align}
This is the change in the probability of the particle being in the $i^\up{th}$ state during that time interval. Tully identifies each term in the sum as a probability flow between a pair of states, and if non-negative, equates each term with the (unconditional) probability of a transition from the $j^\up{th}$ state to the $i^\up{th}$ state. We do the same, except that we identify two transition probabilities, one due to the spatial variation in the eigenbasis, and one due to the temporal variation. This gives us transition probability matrix elements:
\begin{align}
P^\up{space}_{ij} &= \max\left\{0, 
 2\dd t\re\left(c_i^*(t)
\matrixel{\chi_i(\vec r, t)}{\vec v\cdot\nabla}{\chi_j(\vec r, t)}
 c_j(t)\right)\right\},\\
P^\up{time}_{ij} &= \max\left\{0, 
 2\dd t\re\left(c_i^*(t)
\matrixel{\chi_i(\vec r, t)}{\partial_t}{\chi_j(\vec r, t)}
 c_j(t)\right)\right\}
\end{align}
for transitions of the hidden variable from the $j^\up{th}$ to the $i^\up{th}$ eigenstate of $\hat H(\vec r, t)$ during the time interval $\dd t$ due to non-adiabatic spatial and temporal variations in $\hat H(\vec r, t)$ respectively.
...
S^\up{time}_{ij} &= \frac1{\abs{c_j(t)}^2} P^\up{space}_{ij}.
\end{align}
Note that individually, these two matrices are not (left) stochastic matrices, since their columns do not individually sum to unity. Their sum is a (left) stochastic matrix however.

During a simulation, to choose whether a transition occurs due to spatial or temporal variations in the Hamiltonian, one should not make independent random choices based on the two above matrices. Rather one should assemble the probabilities of possible events---transitions from the current state to all others via both spatial and temporal non-adiabatic transitions---into a single list of probabilities, and then take the cumulative sum, resulting in a list of numbers between zero and one. A randomly generated number between zero and one can then be used to determine which event occurs, with the correct probability (\figref{fig:random_choice})

\begin{figure}[t]
    \centerfloat
    \includegraphics{figures/hidden_variables/random_choice.pdf}
    \caption{Probabilistically choosing a transition. Here the hidden variable is in the $j^\up{th}$ state and we are choosing whether it will transition to state $1$ or $2$ (one of which is the state it is already in), and whether it will transition due to a spatial or temporal non-adiabatic transition. All the probabilities for what may happen in a timestep sum to one, so an array of the cumulative probabilities can be constructed, and a random number drawn from the range $[0,1]$. The index of the largest element of the array of cumulative sums that the random number is smaller than corresponds to the the event to occur. In the above diagrammatic example, the result of the random draw is that the hidden variable is to transition to state $1$ via a temporal non-adiabatic transition.}\label{fig:random_choice}
\end{figure}

\subsection{Framing fewest-switches as a hidden-variable theory}
...
Since only the matrix $H_\up{eff}(\vec r, t)\,\dd t$ is required to compute transition probabilities according to~\eqref{eq:fewest_switches_Heff}, and not its component terms,\footnote{Though we do need its component terms if we wish to distinguish between spatially vs temporally induced transitions as in [SECREF].} specifying the initial state vector and unitary for an interval of time evolution (both in the local basis) is a sufficient input to be able to compute transition probabilities using Tully's fewest-switches algorithm. The resulting matrix of probabilities then corresponds to the matrix we (and Aaronson) have been calling $P$ in the context of hidden-variable theories. The corresponding stochastic matrix $S$ can easily be obtained by scaling the columns of $P$ by the state populations as in~\eqref{eq:P_matrix_def}. Tully's fewest-switches algorithm thus satisfies Aaronson's definition of a hidden-variable theory. What is somewhat remarkable is the low computational complexity of computing probabilities via fewest-switches. There is no matrix scaling, no matrix permanents, or any other large computational expense. It is not clear how many of Aaronson's axioms are satisfied by fewest-switches, but since it does depend on the actual coupling strengths between states via the non-adiabatic Hamiltonian, one would expect it to satisfy a few of them.
...
One method is to compute a unitary as usual for the total evolution of the state vector (excluding decoherence) during a timestep, and compute $S^\up{total}$ from that according to one's hidden variable theory. Then, repeat this step, but with a unitary computed by holding the Hamiltonian's explicit time dependence constant over the timestep (allowing $\vec r$ to still vary as normal), to obtain the conditional transition probabilities $S^\up{space}$ for non-adiabatic transitions due to spatial variations alone.

[NOT SURE THIS MAKES SENSE, THINK ABOUT IT MORE]
