...
As shown in \figref{fig:HVSC_schematic}, one sees the spin-up population decreasing over time until only spin-up population remains. The rate at which spin-up population `leaves' the region of space we are watching depends on how quickly the wavepackets are accelerating away from each other, as well as the shape of the wavepackets. Likewise, if one follows the spin-up component instead, one sees the spin-down population decrease to zero.
...
To obtain an explicit form for the decoherence factor $R_{ij}(t)$, we need to start imposing our assumptions about the motional states $\{\ket{\phi_i(t)}\}$. As alluded to earlier, we will be assuming that each motional state $\ket{\phi_i(t)}$ is a Gaussian wavepacket propagating---without dispersion---with centre-of-mass motion evolving classically according to the adiabatic potential $V_i(\vec r, t)$ (defined in Section \ref{sec:fewest_switches}). [TODO make sure the single index notation defined there]. Thus the wavefunctions of these motional states are
\begin{align}
...
\end{align}
Where $A$ is a real normalisation constant,\footnote{The overall phase is determined by the offset $\vec r - \vec r_i(t)$ in the second term in the exponent, and is chosen such that $\phi_i(\vec r, t)$ is real at $\vec r=\vec r_i$, which ensures that $\braket{\phi_i(t)}{\phi_j(t)}$ has a phase depending only on the relative positions of the two wavepackets rather than their distance from some arbitrary origin. Acquiring an additional arbitrary phase at each timestep when we apply our reduction operator $\hat R$ would not affect the system dynamics, but is unappealing nonetheless.} $\vec r_{ij}(t) = \vec r_i(t) - \vec r_j(t)$ and $\vec k_{ij}(t) = \frac m\hbar \left(\vec v_i(t) - \vec v_j(t)\right)$ are the relative mean position and wavevector of the pair of wavepackets, and where the centre-of-mass position and velocity of each wavepacket evolve classically according to
\begin{align}
\dv{t} \vec r_i(t) &= \vec v_i(t)\\
\dv{t} \vec v_i(t) &= -\frac1m\nabla V_i(\vec r_i, t).
\end{align}
This gives for the decoherence factor:
\begin{align}\label{eq:decoherence_factor_const_a}
...
R_{ij}(t) = \exp\left[-\frac {\abs{\vec a_{ij}}^2} 2 \left(\frac1 {16\sigma^2}t^4 + \ii\frac m{2\hbar}t^3 + \frac{m^2\sigma^2}t^2\hbar\right)\right].
\end{align}
...
\begin{align}
\Gamma_i^\up{Markov} = -\frac{\left[\dv{t}\int_{-\infty}^0 R(t-t^\prime) \,\dd t^\prime\right]_{t=0} }
...
\begin{align}
\left(\Gamma_i^\up{Markov}\right)^{-1} &= 
  \int_0^\infty R(t^\prime) \,\dd t^\prime\\
...
[TODO scrap the following sentence, introduce it some other way, and continue to copy and paste from the other section where this is completed. Get the figure!]
The expression~\eqref{eq:decoherence_factor_const_a} requires a relative acceleration, for which we use the relative acceleration between the pair of adiabatic potentials at the current moment in time and current position of the atom during a simulation:
\begin{align}
\vec a_{ij} = -\frac1 m\left(\nabla V_i(\vec r, t) - \nabla V_i(\vec r, t)\right).
\end{align}



\subsection{Continuously spawned mean auxiliary trajectories}\label{sec:spawned_trajectories}
