...
In Subsection~\ref{sec:backaction} I show the general mechanism by which the divergence of trajectories decoheres internal states of an atom, and specifically how this appears as an apparent decrease in the amplitudes of each other state when one imposes the adiabatic trajectory corresponding to one specific internal state. I then show in Subsection~\ref{sec:zeno_effect} why naively projecting the wavefunction onto a single trajectory at all times yields nonsensical results, and then in Subsections~\ref{sec:markovian_decoherence} and~\ref{sec:markovian_decoherence} I present two remedies for how to approximately include decoherence despite this. 
...
R_{ij}(t) = \exp\left[-\frac {\abs{\vec a_{ij}}^2} 2 \left(\frac1 {16\sigma^2}t^4 + \ii\frac m{2\hbar}t^3 + \frac{m^2\sigma^2}\hbar t^2 \right)\right].
\end{align}
...
\frac1{\gamma_{ij}^\up{Markov}(\vec r, t)} &=
  \int_0^\infty 
    \exp\left[
      -\frac {\abs{\vec a_{ij}(\vec r, t)}^2} 2 \left(\frac1 {16\sigma^2}{t^\prime}^4 + \ii\frac m{2\hbar}{t^\prime}^3 + \frac{m^2\sigma^2}\hbar {t^\prime}^2 \right)
      \right]
    \,\dd t^\prime
\end{align}
...
This gives us:
[TODO REWRITE WITH T, NOT X AND K]
\begin{align}
\frac1{\gamma_{ij\,\up{(pos)}}^\up{Markov}(\vec r, t)} &\approx 
  \int_0^\infty \exp\left[
        -\frac{1} {8\sigma^2} \abs{\vec r_{ij}}^2
      - \frac \ii2 \vec r_{ij} \cdot \vec k_{ij}
      \right]\, \dd t^\prime\\
      &\approx \int_0^\infty \exp\left[-
              \frac{1} {8\sigma^2} \abs{\vec r_{ij}}^2\right]\left(1 - \frac \ii2 \vec r_{ij} \cdot \vec k_{ij}\right)\, \dd t^\prime\label{eq:gamma_pos_exp}\\
...
\begin{align}
\gamma_{ij\,\up{(pos)}}^\up{Markov} &\approx \frac1{2^{\frac54}\upGamma(\tfrac54)}\sqrt{\frac{\abs{\vec a_{ij}(\vec r, t)}}{\sigma}}
+ \frac \ii {2\sqrt{2}\upGamma(\tfrac54)^2} \frac{m\sigma \abs{\vec a_{ij}(\vec r, t)}}{\hbar}\label{eq:gamma_pos_final}%\\
