...
\end{align}
\footnote{Which, if no approximations are made, is infinite dimensional - the wavefunction has one complex number for each of the infinite number of points in the two dimensional region.} The state vector can then be represented as a column vector, with one complex number giving the amplitude of each component of the state vector in the chosen basis. Lets say we discretise space in a $5\times 5$ grid,\footnote{For the sake of example---$256\times256$ is a more realistic minimum.} and only represent the wavefunction at those $25$ points in space. The state vector can then be represented by a list of $25$ complex numbers. This is our column vector.
...
But at what point did we choose a basis just now? Imagine
