...
\end{align}
in this case, the Schr\"odinnger equation can be transformed into a form that is often more computationally tractable and analytically useful, by treating part of the evolution due to the time-independent part analytically. This transformation is called an \emph{interaction picture} \cite[p.~317]{sakurai}, or sometimes, particularly in atomic physics, a \emph{rotating frame}, due to the fact that for a spin-$\frac12$ system in a constant magnetic field, the action of $H_0$ entails literal rotation of the spin vector in three dimensional space. 

To analytically remove the action of $\hat H_0$, we define the state vector $\ket{\psi(t)}$ as the action of $\hat H_0$ on an \emph{interaction picture state vector} $\ket{\psi_\up{I}(t)}$:
\begin{align}
\ket{\psi(t)} = \ee^{-\frac\ii\hbar\hat H t}\ket{\psi_\up{I}(t)}
\end{align}
and substitute into the Schr\"odinger equation, in order to obtain the equation of motion for the interaction picture state vector $\ket{\psi_\up{I}(t)}$:
\begin{align}
