...
To that end, we decompose $\hat H(t)$ into diagonal and non-diagonal (in the $\ket{n}$ basis) parts at each moment in time:
\begin{align}
...
This use of \textsc{fft}s in both the \textsc{fss} and \textsc{rk4ip} methods necessarily imposes periodic boundary conditions on a simulation, which may not be desired. By contrast, if different boundary conditions are desired, finite differences instead of \textsc{fft}s can be used to evaluate spatial derivatives in the \textsc{rk4} and \text{rk4ip} methods. Finite differences seem to have waned in popularity in \textsc{bec} simulations due to the ``infinite order" accuracy of using \textsc{fft}s for derivatives, but from what I can gather the impression that finite differences are inaccurate stems from only considering second order finite differences [cite FEDVR paper]. As shown in [TODO CITE EARLIER SECTION], 6th or so order finite differences are entirely sufficient for \textsc{bec} simulations.
