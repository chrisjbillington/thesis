...
\linespread{1.5}
\footnotesize
\Function{$\mathrm{RK4ILIP}$}{$t_0$, $\Delta t$, $\psi_0^{(n)}$, $F$}
    ...
    \State $\phi_1^{(n)} \gets \psi_0^{(n)} + k_1^{(n)} \frac{\Delta t}{2}$
    \Comment{First RK4 estimate of IP state vector, at $t=t_0 + \frac{\Delta t}{2}$}
    \State $\psi_1^{(n)} \gets e^{-i\omega^{(n)}\frac{\Delta t}{2}}\phi_1^{(n)}$
    \Comment{Convert first estimate back to SP}
    \State $f_2^{(n)} \gets F(t_0 + \frac{\Delta t}{2}, \psi_1^{(n)})$
    \Comment{Second evaluation of Schrödinger picture DE}
    \State $k_2^{(n)} \gets e^{i\omega^{(n)}\frac{\Delta t}{2}}f_2^{(n)} + i\omega^{(n)}\phi_1^{(n)}$
    \Comment{Evaluate (\ref{eq:IPDE_components}) with $t-t^\prime=\frac{\Delta t}{2}$}
    \State $\phi_2^{(n)} \gets \psi_0^{(n)} + k_2^{(n)} \frac{\Delta t}{2}$
    \Comment{Second RK4 estimate of IP state vector, at $t=t_0 + \frac{\Delta t}{2}$}
    \State $\psi_2^{(n)} \gets e^{-i\omega^{(n)}\frac{\Delta t}{2}}\phi_2^{(n)}$
    \Comment{Convert second estimate back to SP}
    \State $f_3^{(n)} \gets F(t_0 + \frac{\Delta t}{2}, \psi_2^{(n)})$
    \Comment{Third evaluation of Schrödinger picture DE}
    \State $k_3^{(n)} \gets e^{i\omega^{(n)}\frac{\Delta t}{2}}f_3^{(n)} + i\omega^{(n)}\phi_2^{(n)}$
    \Comment{Evaluate (\ref{eq:IPDE_components}) with $t-t^\prime=\frac{\Delta t}{2}$}
    \State $\phi_3^{(n)} \gets \psi_0^{(n)} + k_3^{(n)} \Delta t$
    \Comment{Third RK4 estimate of IP state vector, at $t=t_0 + \Delta t$}
    \State $\psi_3^{(n)} \gets e^{-i\omega^{(n)}\Delta t}\phi_3^{(n)}$
    \Comment{Convert third estimate back to SP}
    \State $f_4^{(n)} \gets F(t_0 + \Delta t, \psi_3^{(n)})$
    \Comment{Fourth evaluation of Schrödinger picture DE}
    \State $k_4^{(n)} \gets e^{i\omega^{(n)}\Delta t}f_4^{(n)} + i\omega^{(n)}\phi_3^{(n)}$
    \Comment{Evaluate (\ref{eq:IPDE_components}) with $t-t^\prime=\Delta t$}
    \State $\phi_4 \gets
            \psi_0 + \frac{\Delta t}{6}\left(k_1^{(n)} + 2k_2^{(n)} + 2k_3^{(n)} + k_4^{(n)}\right)$
    \Comment{Fourth RK4 estimate, at $t=t_0 + \Delta t$}
    \State $\psi_4^{(n)} \gets e^{-i\omega^{(n)}\Delta t}\phi_4^{(n)}$
    \Comment{Convert fourth estimate back to SP}
    \State \Return $\psi_4^{(n)}$
    \Comment{Return the computed SP state vector at $t=t_0 + \Delta t$}
\EndFunction
\end{algorithmic}
...
\newgeometry{left=2.54cm, bottom=1.5in}
\begin{figure}[ht!]
\centerfloat
