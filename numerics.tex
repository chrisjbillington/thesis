...
The fact that the finite difference matrices are banded allows them to applied as a ``stencil" to a discrete state vector, computing an approximation to some linear sum of derivative operators at each point of discrete space by consideration of only that point and a small number of surrounding point. For example, a $\Ord{\upDelta x^2}$ approximation to the kinetic energy operator may be evaluated at each point as:
\begin{align}
U^\dagger_{\textsc{dft2}} \frac{\hbar^2 (k_x^2 + k_y^2)}{2m} U_{\textsc{dft2}}\psi(\tilde{\vec r}) = 
-\ii\delta^{(n)}_{\upDelta x} \otimes \mathbb{I}_{n_y}\psi(\tilde{\vec r}) + \Ord{\upDelta x^n}
\end{align} 

[bandedness and the fact that each operator acts only in one dimension means they can be applied as a stencil, for example the laplacian:]

[finite differences also compatible with non-uniformly spaced grids, leading to different matrix elements \cite{fornberg_generation_1988}, and breaking the interpretation of them as approximations to the Fourier method of computing derivatives, since discrete fourier transforms require uniformly spaced grids. Non-uniform grids can be very useful for some physics problems, such as modelling electron wavefunctions in a coulomb potential, in which energy scales are much larger, and hence wavelengths smaller, at small distances from the centre of the potential than large distances. For those problems one could transform the schrodinger equation into spherical coordinates and have a non-uniform grid in the radial direction. This point is made by [cite FEDVR paper] in the context of a finite element method [see section blah], but it's worth noting that finite differences, which are simpler to implement than finite elements, are also compatible with non uniform grids such as this.]

\begin{align}
\delta^{(2)}_{x, \upDelta x}\left[\psi\right](\vec r_{ij}) = 
\frac{1}{2\upDelta x}\left(\psi(x_{i + 1}, y_j) - \psi(x_{i - 1}, y_j)\right)\\
\frac{1}{2\upDelta x}\left(\psi(x_{i + 1}, y_j) - \psi(x_{i - 1}, y_j)\right)\\
\end{align}
...
\begin{itemize}
    ...
    \item Computationally efficient compared to FFTs in split-step (banded matrices amenable to splitting into sum of two block-diagonal matrices).
