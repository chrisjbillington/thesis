...
x_interp = np.linspace(0, 7, nx, endpoint=False).reshape((1, nx))
y_interp = np.linspace(0, 7, nx, endpoint=False).reshape((nx, 1))
psi_interp = np.zeros((nx, nx), dtype=complex)
for i in range(n):
    for j in range(n):
        psi_interp += np.exp(1j*ky[i]*y_interp)*np.exp(1j*kx[j]*x_interp)*f_psi[i, j]

FIG_WIDTH = 4.5
FIG_HEIGHT = 3.25

fig = plt.figure(figsize=(FIG_WIDTH, FIG_HEIGHT))
gs = gridspec.GridSpec(4, 1, left=0.075, bottom=0.15,
...
plt.imshow(psi_interp.real, extent=[0, 7, 0, 7], origin='lower', cmap=plt.get_cmap('seismic'),
           vmin=-1.0, vmax=1.0)
...
plt.plot(x_interp.flatten(), psi_interp[int(round(1024*2/7))].real, color=colors['red'])
plt.grid(True)
plt.axis([0, 7, -0.3, 1.1])
plt.xlabel('$x$')
...
print(ax1_bbox.x0)
ax2.set_position([0.291589, ax2_bbox.y0, ax1_bbox.width, ax2_bbox.height])
