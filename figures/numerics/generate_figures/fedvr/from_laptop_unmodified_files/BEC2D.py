...
LEFT_BORDER = (FIRST, ALL, FIRST, ALL) # The points on the left edge of the left border element
RIGHT_BORDER = (LAST, ALL, LAST, ALL) # The points on the right edge of the right border element
BOTTOM_BORDER = (ALL, FIRST, ALL, FIRST) # The points on the bottom edge of the bottom border element
TOP_BORDER = (ALL, LAST, ALL, LAST) # The points on the top edge of the top border element
INTERIOR_ELEMENTS = (INTERIOR, INTERIOR, ALL, ALL) # All points in all elements that are not border elements
...
class Simulator2D(object):
    def __init__(self, x_min_global, x_max_global, y_min_global, y_max_global,
                 n_elements_x_global, n_elements_y_global, Nx, Ny, n_components, periodic_x=True, periodic_y=True,
        ...
        self.EDGE_POINTS_X = np.s_[::self.Nx-1]
        self.EDGE_POINTS_Y = np.s_[::self.Ny-1]
        self.BORDER_ELEMENTS_X = np.s_[::self.n_elements_x-1]
        self.BORDER_ELEMENTS_Y = np.s_[::self.n_elements_y-1]

        # These are for indexing all edges of all border elements. The below
        # four sets of slices used in succession cover the edges of border
        # elements exactly once:
        self.BORDER_ELEMENTS_X_EDGE_POINTS_X = (self.BORDER_ELEMENTS_X, ALL, self.EDGE_POINTS_X, ALL)
        self.BORDER_ELEMENTS_Y_EDGE_POINTS_X = (INTERIOR, self.BORDER_ELEMENTS_Y, self.EDGE_POINTS_X, ALL)
        self.BORDER_ELEMENTS_X_EDGE_POINTS_Y = (self.BORDER_ELEMENTS_X, ALL, INTERIOR, self.EDGE_POINTS_Y)
        self.BORDER_ELEMENTS_Y_EDGE_POINTS_Y = (INTERIOR, self.BORDER_ELEMENTS_Y, INTERIOR, self.EDGE_POINTS_Y)

        # These are for indexing all edges of non-border elements. Used in
        # succession they cover the edges of these elements exactly once:
        self.INTERIOR_ELEMENTS_EDGE_POINTS_X = (INTERIOR, INTERIOR, self.EDGE_POINTS_X, ALL)
        self.INTERIOR_ELEMENTS_EDGE_POINTS_Y = (INTERIOR, INTERIOR, INTERIOR, self.EDGE_POINTS_Y)

        # These are for indexing all points of border elements. Used in
        # succession they cover the these elements exactly once:
        self.BORDER_ELEMENTS_X_ALL_POINTS = (self.BORDER_ELEMENTS_X, ALL, ALL, ALL)
        self.BORDER_ELEMENTS_Y_ALL_POINTS = (INTERIOR, self.BORDER_ELEMENTS_Y, ALL, ALL)
    ...
    def _setup_MPI_grid(self):
        ...
        if self.MPI_x_coord > 0 or self.periodic_x:
            ...
            self.left_border_is_zero = False
        else:
            self.MPI_rank_left = MPI.PROC_NULL
            self.left_border_is_zero = True
        if self.MPI_x_coord < self.MPI_size_x -1 or self.periodic_x:
            ...
            self.right_border_is_zero = False
        else:
            self.MPI_rank_right = MPI.PROC_NULL
            self.right_border_is_zero = True
        if self.MPI_y_coord > 0 or self.periodic_y:
            ...
            self.bottom_border_is_zero = False
        else:
            self.MPI_rank_down = MPI.PROC_NULL
            self.bottom_border_is_zero = True
        if self.MPI_y_coord < self.MPI_size_y -1 or self.periodic_y:
            ...
            self.top_border_is_zero = False
        else:
            self.MPI_rank_up = MPI.PROC_NULL
            self.top_border_is_zero = True
    ...
    def MPI_send_border_kinetic(self, Kx_psi, Ky_psi):
        ...
        self.MPI_left_kinetic_send_buffer[:] = Kx_psi[LEFT_BORDER].reshape(self.n_elements_y * self.Ny)
        self.MPI_right_kinetic_send_buffer[:] = Kx_psi[RIGHT_BORDER].reshape(self.n_elements_y * self.Ny)
        self.MPI_bottom_kinetic_send_buffer[:] = Ky_psi[BOTTOM_BORDER].reshape(self.n_elements_x * self.Nx)
        self.MPI_top_kinetic_send_buffer[:] = Ky_psi[TOP_BORDER].reshape(self.n_elements_x * self.Nx)
    ...
    def MPI_receive_border_kinetic(self, Kx_psi, Ky_psi):
        ...
        Kx_psi[LEFT_BORDER] += left_data
        Kx_psi[RIGHT_BORDER] += right_data
        Ky_psi[BOTTOM_BORDER] += bottom_data
        Ky_psi[TOP_BORDER] += top_data
    ...
    def MPI_send_border_values(self, psi):
        ...
        not be."""
        self.MPI_left_values_send_buffer[:] = psi[LEFT_BORDER].reshape(self.n_elements_y * self.Ny)
        self.MPI_bottom_values_send_buffer[:] = psi[BOTTOM_BORDER].reshape(self.n_elements_x * self.Nx)
    ...
    def MPI_receive_border_values(self, psi):
        ...
        psi[RIGHT_BORDER] = right_data
        psi[TOP_BORDER] = top_data
    ...
    def global_dot(self, vec1, vec2):
        ...
        # Never count right/top edges, to avoid double counting. Also Don't
        # count left/bottom edges if they are zero due to boundary conditions:
        if self.left_border_is_zero and self.bottom_border_is_zero:
            # Have to do four separate slices to exlude the bottom and left boundaries.
            point_selections = (
                # Bottom left corner element, excluding bottom and left boundary points:
                (FIRST, FIRST, INTERIOR, INTERIOR),
                # Left border elements, excluding bottom left element, and excluding left boundary points:
                (FIRST, ALL_BUT_FIRST, INTERIOR, ALL_BUT_LAST),
                # Bottom border elements, excluding bottom left element, and excluding bottom boundary points:
                (ALL_BUT_FIRST, FIRST, ALL_BUT_LAST, INTERIOR),
                # All remaining elements:
                (ALL_BUT_FIRST, ALL_BUT_FIRST, ALL_BUT_LAST, ALL_BUT_LAST))
        elif self.left_border_is_zero:
            # Have to do two separate slices to exlude left boundary.
            point_selections = (
                # Left border elements, excluding left boundary points:
                (FIRST, ALL, INTERIOR, ALL_BUT_LAST),
                # All remaining elements:
                (ALL_BUT_FIRST, ALL, ALL_BUT_LAST, ALL_BUT_LAST))
        elif self.bottom_border_is_zero:
            # Have to do two separate slices to exlude bottom boundary.
            point_selections = (
                # Left border elements, excluding left boundary points:
                (ALL, FIRST, ALL_BUT_LAST, INTERIOR),
                # All remaining elements:
                (ALL, ALL_BUT_FIRST, ALL_BUT_LAST, ALL_BUT_LAST))
        else:
            # All elements, with only the top and right edges of each element
            # excluded to avoid double counting (which is the case with all
            # the above slices as well)
            point_selections = ((ALL, ALL, ALL_BUT_LAST, ALL_BUT_LAST),)

        local_dot = 0
        for slices in point_selections:
            local_dot += np.vdot(vec1[slices], vec2[slices]).real
    ...
    def compute_H(self, t, psi, H, border_element_slices=(), internal_element_slices=(),
        ...
        border_element_slices and internal_element slices can be provided to
        ...
        if outarrays is None:
            Kx_psi = np.zeros(psi.shape, dtype=psi.dtype)
            Ky_psi = np.zeros(psi.shape, dtype=psi.dtype)
            K_psi = np.zeros(psi.shape, dtype=psi.dtype)
            U_psi = np.zeros(psi.shape, dtype=psi.dtype)
            U_nonlinear = np.zeros(psi.shape, dtype=psi.dtype)
        else:
            Kx_psi, Ky_psi, K_psi, U_psi, U_nonlinear = outarrays

        # Compute H_psi at the border elements first, before firing off data
        ...
        # possible. If the caller has provided border_element_slices and
        # internal_element_slices, then don't evaluate all all the points, just at the
        # ones requested. Basically pre_MPI_slices must contain any border
        ...
        if not (border_element_slices or internal_element_slices):
            border_element_slices = (self.BORDER_ELEMENTS_X_ALL_POINTS, self.BORDER_ELEMENTS_Y_ALL_POINTS)
            internal_element_slices = (INTERIOR_ELEMENTS,)

        # array = np.zeros(self.shape)
        # for slices in border_element_slices:
        #     array[slices] += 1
        # for slices in internal_element_slices:
        #     array[slices] += 0.5

        # array = array.transpose(0, 2, 1, 3, 4, 5).reshape((self.n_elements_x*self.Nx, self.n_elements_y*self.Ny))
        ...
        #     plt.show()
        # plt.clf()
        # plt.imshow(array.transpose(), origin='lower', cmap=cm.jet, vmin=0, vmax=1, interpolation='nearest')
        # plt.draw()


        # Evaluate H_psi at the border element slices, if any:
        for slices in border_element_slices:
            x_elements, y_elements, x_points, y_points = slices

            # if x_elements is self.BORDER_ELEMENTS_X and y_elements is ALL:
            #     print('x_elements is BORDER_ELEMENTS_X:  ', x_elements)
            #     print('y_elements is ALL:                ', y_elements)
            # elif x_elements is INTERIOR and y_elements is self.BORDER_ELEMENTS_Y:
            #     print('x_elements is INTERIOR:           ', x_elements)
            #     print('y_elements is BORDER_ELEMENTS_Y:  ', y_elements)
            # else:
            #     raise NotImplementedError
            # if x_points is ALL:
            #     print('x_points is ALL:                  ', x_points)
            # elif x_points is self.EDGE_POINTS_X:
            #     print('x_points is EDGE_POINTS_X:        ', x_points)
            # elif x_points is INTERIOR:
            #     print('x_points is INTERIOR:             ', x_points)
            # else:
            #     if x_points.stop - x_points.start == 1:
            #         print('x_points is a single point:   ', x_points)
            #     else:
            #         print(x_points)
            #         raise NotImplementedError
            # if y_points is ALL:
            #     print('y_points is ALL:                  ', y_points)
            # elif y_points is self.EDGE_POINTS_Y:
            #     print('y_points is EDGE_POINTS_Y:        ', y_points)
            # elif y_points is INTERIOR:
            #     print('y_points is INTERIOR:             ', y_points)
            # else:
            #     if y_points.stop - y_points.start == 1:
            #         print('y_points is a single point:   ', y_points)
            #     else:
            #         print(y_points)
            #         raise NotImplementedError
            # print()
            ...
            if x_elements is self.BORDER_ELEMENTS_X:
                # Consider the left and right borders separately in case either is a boundary:
                if self.left_border_is_zero:
                    # Exclude the left boundary points from the application of Kx:
                    if x_points is ALL:
                        Kx_psi[FIRST, y_elements, ALL_BUT_FIRST, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                            Kx[..., ALL_BUT_FIRST, :, :, ALL_BUT_FIRST], psi[FIRST, y_elements, ALL_BUT_FIRST, y_points])
                    elif x_points is self.EDGE_POINTS_X:
                        Kx_psi[FIRST, y_elements, LAST, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                            Kx[..., LAST, :, :, ALL_BUT_FIRST], psi[FIRST, y_elements, ALL_BUT_FIRST, y_points])
                    else:
                        # x_points is INTERIOR, or a single non-edge point:
                        Kx_psi[FIRST, y_elements, x_points, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                            Kx[..., ALL_BUT_FIRST], psi[FIRST, y_elements, ALL_BUT_FIRST, y_points])
                else:
                    # Evaluate the kinetic term on these elements as normal:
                    Kx_psi[FIRST, y_elements, x_points, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                                                                              Kx,  psi[FIRST, y_elements, :, y_points])
                if self.right_border_is_zero:
                    # Exclude the right boundary points from the application of Kx:
                    if x_points is ALL:
                        Kx_psi[LAST, y_elements, ALL_BUT_LAST, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                            Kx[..., ALL_BUT_LAST, :, :, ALL_BUT_LAST], psi[LAST, y_elements, ALL_BUT_LAST, y_points])
                    elif x_points is self.EDGE_POINTS_X:
                        Kx_psi[LAST, y_elements, FIRST, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                            Kx[..., FIRST, :, :, ALL_BUT_LAST], psi[LAST, y_elements, ALL_BUT_LAST, y_points])
                    else:
                        # x_points is INTERIOR, or a single non-edge point:
                        Kx_psi[LAST, y_elements, x_points, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                            Kx[..., ALL_BUT_LAST], psi[LAST, y_elements, ALL_BUT_LAST, y_points])
                else:
                    # Evaluate the kinetic term on these elements as normal:
                    Kx_psi[LAST, y_elements, x_points, y_points] = np.einsum('...nmci,...imcq->...nmcq',
                                                                             Kx,  psi[LAST, y_elements, :, y_points])
            elif x_elements is INTERIOR:
                # Evaluate the kinetic term on these elements as normal:
                Kx_psi[slices] = np.einsum('...nmci,...imcq->...nmcq', Kx,  psi[x_elements, y_elements, :, y_points])
            else:
                raise NotImplementedError

            if y_elements is ALL or y_elements is self.BORDER_ELEMENTS_Y:
                # Consider the bottom and top borders separately in case either is a boundary:
                if self.bottom_border_is_zero:
                    if y_points is ALL:
                        Ky_psi[x_elements, FIRST, x_points, ALL_BUT_FIRST] = np.einsum('...mcj,...jcq->...mcq',
                            Ky[..., ALL_BUT_FIRST, :, ALL_BUT_FIRST], psi[x_elements, FIRST, x_points, ALL_BUT_FIRST])
                    elif y_points is self.EDGE_POINTS_Y:
                        Ky_psi[x_elements, FIRST, x_points, LAST] = np.einsum('...mcj,...jcq->...mcq',
                            Ky[..., LAST, :, ALL_BUT_FIRST], psi[x_elements, FIRST, x_points, ALL_BUT_FIRST])
                    else:
                        # y_points is INTERIOR, or a single non-edge point:
                        Ky_psi[x_elements, FIRST, x_points, y_points] = np.einsum('...mcj,...jcq->...mcq',
                            Ky[..., ALL_BUT_FIRST], psi[x_elements, FIRST, x_points, ALL_BUT_FIRST])
                else:
                    # Evaluate the kinetic term on these elements as normal:
                    Ky_psi[x_elements, FIRST, x_points, y_points] = np.einsum('...mcj,...jcq->...mcq',
                                                                          Ky,  psi[x_elements, FIRST, x_points, :])
                if self.top_border_is_zero:
                    if y_points is ALL:
                        Ky_psi[x_elements, LAST, x_points, ALL_BUT_LAST] = np.einsum('...mcj,...jcq->...mcq',
                            Ky[..., ALL_BUT_LAST, :, ALL_BUT_LAST], psi[x_elements, LAST, x_points, ALL_BUT_LAST])
                    elif y_points is self.EDGE_POINTS_Y:
                        Ky_psi[x_elements, LAST, x_points, FIRST] = np.einsum('...mcj,...jcq->...mcq',
                            Ky[..., FIRST, :, ALL_BUT_LAST], psi[x_elements, LAST, x_points, ALL_BUT_LAST])
                    else:
                        # y_points is INTERIOR, or a single non-edge point:
                        Ky_psi[x_elements, LAST, x_points, y_points] = np.einsum('...mcj,...jcq->...mcq',
                            Ky[..., ALL_BUT_LAST], psi[x_elements, LAST, x_points, ALL_BUT_LAST])
                else:
                    # Evaluate the kinetic term on these elements as normal:
                    Ky_psi[x_elements, LAST, x_points, y_points] = np.einsum('...mcj,...jcq->...mcq',
                                                                          Ky,  psi[x_elements, LAST, x_points, :])
                if y_elements is ALL:
                    # And now the interior elements in the y direction:
                    Ky_psi[x_elements, INTERIOR, x_points, y_points] = np.einsum('...mcj,...jcq->...mcq',
                                                                          Ky,  psi[x_elements, INTERIOR, x_points, :])
            else:
                raise NotImplementedError

            U_psi[slices] = np.einsum('...cl,...lq->...cq', U,  psi[slices])

        if border_element_slices and sum_at_edges:
    ...
    def find_groundstate(self, psi_guess, H, mu, t=0, convergence=1e-12, relaxation_parameter=1.7,
        ...
        Kx_diags = np.einsum('...nmcn->...nmc', Kx).copy()[np.newaxis, ..., np.newaxis]
        Ky_diags = np.einsum('...mcm->...mc', Ky).copy()[..., np.newaxis]
        ...
        U_diags = np.einsum('...cc->...c', U).copy()[..., np.newaxis]
        ...
        # All the slices for covering the edges of the border elements and internal elements:
        BORDER_ELEMENT_EDGES = (self.BORDER_ELEMENTS_X_EDGE_POINTS_X, self.BORDER_ELEMENTS_Y_EDGE_POINTS_X,
                                   self.BORDER_ELEMENTS_X_EDGE_POINTS_Y, self.BORDER_ELEMENTS_Y_EDGE_POINTS_Y)
            ...
            for slices in point_selections:
                if slices is EDGE_POINT_SLICES:
                    ...
                    self.compute_H(t, psi, H,
                                   border_element_slices=BORDER_ELEMENT_EDGES,
                    ...
                    point_slices = slices[2:]

                    # Slices to get this point in the border elements:
                    border_element_slices = ((self.BORDER_ELEMENTS_X, ALL) + point_slices,
                                             (INTERIOR, self.BORDER_ELEMENTS_Y) + point_slices)

                    # A slices to get this point in the internal elements:
                    internal_element_slices = ((INTERIOR, INTERIOR) + point_slices,)

                    self.compute_H(t, psi, H,
                                   border_element_slices=border_element_slices,
                                   internal_element_slices=internal_element_slices,
                                   sum_at_edges=False,
                                   outarrays=(Kx_psi, Ky_psi, K_psi, U_psi, U_nonlinear))
                ...
                if slices is EDGE_POINT_SLICES:
                    # Impose zero boundary conditions, if applicable. Most of
                    # the work for ensuring we have correct zero boundary
                    # conditions is done in compute_H(), but we also need to
                    # actually set the values to zero, despite them having no
                    # effect on the other points even if we don't.
                    if self.left_border_is_zero:
                        psi[FIRST, ALL, FIRST, ALL] = 0
                    if self.right_border_is_zero:
                        psi[LAST, ALL, LAST, ALL] = 0
                    if self.bottom_border_is_zero:
                        psi[ALL, FIRST, ALL, FIRST] = 0
                    if self.top_border_is_zero:
                        psi[ALL, LAST, ALL, LAST] = 0
    ...
    def evolve(self, psi, H, dt, t_initial=0, t_final=np.inf, imaginary_time=False, mu=0,
        ...
        def dpsi_dt(t, psi):
            K_psi, U_psi, U_nonlinear = self.compute_H(t, psi, H)
            H_psi = K_psi + U_psi + (U_nonlinear - mu)*psi
            if imaginary_time:
        ...
        while t < t_final:
            ...
            if method == 'rk4':
                ...
                # Get the diagonals of the Hamiltonian in the spatial basis,
                # and find the angular frequencies of the corresponding
                # dynamical phase evolution:
                Kx, Ky, U, U_nonlinear = H(t, psi, *ALL_ELEMENTS_AND_POINTS)
                U_diags = np.einsum('...cc->...c', U).copy()[..., np.newaxis]
                omega = (U_diags + U_nonlinear)/self.hbar

                if imaginary_time:
                    omega = -1j*omega

                # Use these angular frequencies to define an interacton
                # picture, and compute the unitary transformations for
                # switching between pictures at the various substeps of rk4:
