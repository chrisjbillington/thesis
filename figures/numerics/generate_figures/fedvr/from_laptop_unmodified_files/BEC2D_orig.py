    ...
    def find_groundstate(self, psi_guess, H, mu, t=0, convergence=1e-12, relaxation_parameter=1.7,
        ...
        while True:
            ...
            for slices in point_selections:
                ...
                if slices is EDGE_POINT_SLICES:
                    # Impose zero boundary conditions on the borders:
                    if self.MPI_x_coord == 0:
                        psi_new_GS[LEFT_BOUNDARY] = 0
                    if self.MPI_x_coord == self.MPI_size_x - 1:
                        psi_new_GS[RIGHT_BOUNDARY] = 0
                    if self.MPI_y_coord == 0:
                        psi_new_GS[BOTTOM_BOUNDARY] = 0
                    if self.MPI_y_coord == self.MPI_size_y - 1:
                        psi_new_GS[TOP_BOUNDARY] = 0
    ...
    def evolve(self, psi, H, dt, t_initial=0, t_final=np.inf, imaginary_time=False,
        ...
        while t < t_final:
            ...
            if self.MPI_x_coord == 0:
                psi[LEFT_BOUNDARY] = 0
            if self.MPI_x_coord == self.MPI_size_x - 1:
                psi[RIGHT_BOUNDARY] = 0
            if self.MPI_y_coord == 0:
                psi[BOTTOM_BOUNDARY] = 0
            if self.MPI_y_coord == self.MPI_size_y - 1:
                psi[TOP_BOUNDARY] = 0
            ...
            k1 = dpsi_dt(psi, t)
            k2 = dpsi_dt(psi + 0.5*k1*dt, t + 0.5*dt)
            k3 = dpsi_dt(psi + 0.5*k2*dt, t + 0.5*dt/3)
            k4 = dpsi_dt(psi + k3*dt, t + dt)
