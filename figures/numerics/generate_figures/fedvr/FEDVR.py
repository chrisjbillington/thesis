...
class Element(object):
    ...
    def __init__(self, N, left_edge=-1, right_edge=1,
        ...
        for point, weight, shapefunction in list(zip(self.points, self.weights, shapefunctions))[1:-1]:
...
class FiniteElements2D(object):
    ...
    def derivative_operators(self):
        ...
        gradx = self.element_x.derivative_operator()
        grady = self.element_y.derivative_operator()
...
class FiniteElements3D(object):
    """A class for operations on an array of identical DVR finite elements in two dimensions"""
    def __init__(self, n_elements_x, n_elements_y, n_elements_z, Nx, Ny, Nz, n_components,
                 left_boundary, right_boundary,
                 bottom_boundary, top_boundary,
                 back_boundary, front_boundary):
        self.n_elements_x = n_elements_x
        self.n_elements_y = n_elements_y
        self.n_elements_z = n_elements_z
        self.Nx = Nx
        self.Ny = Ny
        self.Nz = Nz
        self.n_components = n_components
        ...
        self.element_width_z = (front_boundary - back_boundary)/n_elements_z
        self.element_edges_z = np.linspace(back_boundary, front_boundary, n_elements_z + 1)
        ...
        self.element_z = Element(Nz, 0, self.element_width_z, N_left=Nz, N_right=Nz,
                                 width_left=self.element_width_z, width_right=self.element_width_z)

        # The shape of  vectors. The extra dimension at the end is so that
        # operators on one of our dimensions, which have size > 1 in that
        # dimension, have the same rank as vectors, so that we can say,
        # multiply them easily.
        self.shape = (self.n_elements_x, self.n_elements_y, self.n_elements_z,
