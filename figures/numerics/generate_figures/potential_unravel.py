...
gs = gridspec.GridSpec(1, 2, left=0.075, bottom=0.15,
                        right=0.90, top=0.975, wspace=0.5, hspace=0.075)

plt.subplot(gs[:,0])
plt.gca().tick_params(direction='in')

plt.imshow(V, origin='lower', extent=[0.5, 7.5, 0.5, 7.5])
for i in range(1, n):
...
plt.subplot(gs[:,1])
V_matrix = np.diag(V.ravel())
V_matrix[V_matrix == 0] = np.nan
plt.imshow(V_matrix, extent=[0.5, n**2 + 0.5, n**2 + 0.5, 0.5])
matplotlib.pyplot.yticks([1, 7, 14, 21, 28, 35, 42, 49])
matplotlib.pyplot.xticks([1, 7, 14, 21, 28, 35, 42, 49])

plt.tick_params(axis='y', which='both', left='off', right='on',
                labelright='on', labelleft='off')
plt.ylabel(R"$i_{xy} = i_x + n_xi_y$")
plt.xlabel(R"$i_{xy} = i_x + n_xi_y$")
plt.gca().yaxis.set_label_position("right")

arrow = matplotlib.patches.FancyArrowPatch((0.425, 0.55), (0.55, 0.55), arrowstyle='-|>',
                                           transform=fig.transFigure, mutation_scale=5.0,
                                           facecolor='k')
fig.patches.append(arrow)


plt.savefig('../potential_unravel.pdf', dpi=300)