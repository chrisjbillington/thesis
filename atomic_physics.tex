...
\begin{align}
\hat H_\up{dipole} = -\hat{\vec d}\cdot\vec E,
\end{align}
where $\vec E$ is the classical electric field and $\hat{\vec d}$ is the electric dipole operator:
\begin{align}
\hat{\vec d} = -e\vec{\hat r},
\end{align}
which gives the electric dipole of the atom in terms of the electron's charge $-e$ and its position operator $\hat{\vec r}$. A single matrix element of $\hat H_\up{dipole}$ for coupling a state $\ket{L_J, F, m_F}$ to a state $\ket{L_J^\prime, F^\prime, m_F^\prime}$ is therefore:
\begin{align}
\matrixel{L_J^\prime, F^\prime, m_F^\prime}{\hat H_\up{dipole}}{L_J, F, m_F} = 
\matrixel{L_J^\prime, F^\prime, m_F^\prime}{e\vec{\hat r}\cdot\vec E}{L_J, F, m_F}.
\end{align}
