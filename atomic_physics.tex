...
\begin{align}
\overset{\scriptscriptstyle \mathcal{I}\times{J}}{H}_{\up{D}\,^{87}\up{Rb}}  = 
\left[\begin{matrix}
    \left[\begin{smallmatrix}
        H_{S_{1/2}}\\
    \end{smallmatrix} \right]\\
...
\end{matrix} \right],
\end{align}
where the additional subscripts on the unitary matrices indicates the value of $J$ used in its construction. Armed with the $\{\ket{F, m_F}\}$ basis, we have little remaining reason to use the $\{\ket{m_I}, {m_J}\}$ basis any more. Even the Zeeman interaction, which can't 

[TODO add i cross j as subscripts to all previous matrices in the mI, mJ basis, f subscripts to F matrices. When first mentioning the ]  
